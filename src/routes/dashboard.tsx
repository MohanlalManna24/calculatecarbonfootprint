...
function Dashboard() {
  ...
  const prev = useMemo(
    () =>
      withinDays(store.entries, 14).filter(
        (e) => new Date(e.date).getTime() < Date.now() - 7 * 86400000,
      ),
    [store.entries],
  );
  ...
  return (
    <AppShell>
      <section className="mb-8">
        ...
        <p
          className={`mt-1 inline-flex items-center gap-1 text-sm ${delta <= 0 ? "text-success" : "text-destructive"}`}
        >
        ...
        <article className="rounded-3xl border border-border bg-card p-6">
          ...
          {totalCo2e(
            ...
            <p className="text-sm text-muted-foreground">
              No data yet.{" "}
              <Link to="/track" className="text-primary underline-offset-2 hover:underline">
        ...
        <ul className="grid gap-3 sm:grid-cols-2">
          {recs.map((r) => (
            <li key={r.id} className="rounded-2xl border border-border bg-background p-4">
              ...
              {r.impactKg > 0 && (
                <p className="mt-2 text-sm text-success">~ saves {r.impactKg} kg CO₂e / month</p>
              )}
            </li>
          ))}
        </ul>
