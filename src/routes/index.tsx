...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Track, understand, and reduce your carbon footprint. Private by design, works offline, no account required." },
      { property: "og:title", content: "CarbonWise AI" },
      { property: "og:description", content: "Personal carbon footprint tracking that runs entirely on your device." },
    ],
  }),
  component: Landing,
});
...
function Landing() {
  ...
  function start(e: React.FormEvent) {
    ...
    if (!parsed.success) { setError("Name must be 1–60 characters."); return; }
  ...
  return (
    <div className="min-h-dvh bg-background text-foreground">
      <header className="mx-auto flex max-w-6xl items-center justify-between px-4 py-5 sm:px-6">
        ...
        {store.profile && (
          <Link to="/dashboard" className="rounded-full bg-primary px-4 py-2 text-sm text-primary-foreground hover:bg-primary/90">
            Open dashboard
          </Link>
        )}
      </header>
      ...
      <section className="mx-auto grid max-w-6xl gap-12 px-4 pb-16 pt-10 sm:px-6 md:grid-cols-[1.1fr_1fr] md:gap-16 md:pt-20">
        <div>
          <p className="mb-4 inline-flex items-center gap-2 rounded-full border border-border bg-card px-3 py-1 text-xs text-muted-foreground">
            <span className="size-1.5 rounded-full bg-success" aria-hidden /> Local-first · No account
          </p>
          <h1 className="font-display text-5xl leading-[1.05] text-balance md:text-7xl">
            Your carbon, <em className="not-italic text-primary">measured</em>. Your habits, <em className="not-italic text-accent">transformed</em>.
          </h1>
          ...
          <form onSubmit={start} className="mt-8 flex max-w-md flex-col gap-3 sm:flex-row">
            ...
            />
            <button type="submit" className="rounded-full bg-primary px-6 py-3 text-sm font-medium text-primary-foreground transition-colors hover:bg-primary/90">
              Start tracking
            </button>
          </form>
          ...
          <ul className="mt-10 grid gap-4 sm:grid-cols-2">
            {[
              { icon: Sparkles, t: "Personalized insights", d: "Recommendations tuned to your emission mix." },
              { icon: Target, t: "Goals that stick", d: "Weekly targets per category, with progress." },
        ...
        <aside aria-label="Preview" className="relative">
          <div className="sticky top-10 overflow-hidden rounded-3xl border border-border bg-card shadow-[0_30px_80px_-40px_rgba(31,77,58,0.35)]">
            <div className="border-b border-border bg-secondary/60 px-6 py-4">
              ...
              <p className="font-display text-3xl text-foreground">42.8 <span className="text-base text-muted-foreground">kg CO₂e</span></p>
